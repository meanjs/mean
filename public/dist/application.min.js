...
"use strict";ApplicationConfiguration.registerModule("core",["ui.calendar"]),ApplicationConfiguration.registerModule("core.admin",["core"]),ApplicationConfiguration.registerModule("core.admin.routes",["ui.router"]);
!function(app){"use strict";app.registerModule("events")}(ApplicationConfiguration);
...
"use strict";angular.module("core").config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){$urlRouterProvider.otherwise(function($injector,$location){$injector.get("$state").transitionTo("not-found",null,{location:!1})}),$stateProvider.state("home",{url:"/",templateUrl:"modules/core/client/views/home.client.view.html"}).state("dashboard",{url:"/dashboard",templateUrl:"modules/core/client/views/dashboard.client.view.html"}).state("not-found",{url:"/not-found",templateUrl:"modules/core/client/views/404.client.view.html",data:{ignoreState:!0}}).state("bad-request",{url:"/bad-request",templateUrl:"modules/core/client/views/400.client.view.html",data:{ignoreState:!0}}).state("forbidden",{url:"/forbidden",templateUrl:"modules/core/client/views/403.client.view.html",data:{ignoreState:!0}})}]);
"use strict";angular.module("core").controller("DashboardController",["$scope","$compile","$http","Authentication",function($scope,$compile,$http,Authentication){$scope.authentication=Authentication,$scope.events=[],$scope.editEvent_flag=0,$scope.activeTab="requests",$scope.setActiveTab=function(value){$scope.activeTab=value},$scope.name=null,$scope.date=null,$scope.eTime=null,$scope.sTime=null,$scope.requireTax=null,$scope.acceptEvent_flag=0,$scope.eventList=[{bizName:"Blaze",date:"10/17/17",status:"Pending"},{bizName:"Taco Bell",date:"11/12/23",status:"Confirmed"},{bizName:"Blaze",date:"10/17/17",status:"Pending"},{bizName:"Taco Bell",date:"11/12/23",status:"Confirmed"}],$scope.addEvent=function(title){$scope.events.push({title:title,start:new Date(2017,10,30)})},$scope.deleteEvent=function(event){$http({method:"DELETE",url:"api/events/"+event._id}).then(function(res){console.log("Successful delete"),$scope.loadEventList()},function(res){console.log("Failed delete")})},$scope.requestEvent=function(event){event.organizationsPending.indexOf($scope.authentication.user.displayName)===-1&&(event.organizationsPending.push($scope.authentication.user.displayName),$http({method:"PUT",url:"api/events/"+event._id,data:{organizationsPending:event.organizationsPending}}).then(function(res){console.log("Successful request")},function(res){console.log("Failed request"),console.log(res)}))},$scope.generateStatus=function(event){return event.organizationConfirmed===$scope.authentication.user.displayName?"Accepted":"Pending"},$scope.acceptEvent=function(index){0!==$scope.globalEvent.organizationsPending.length&&$http({method:"PUT",url:"api/events/"+$scope.globalEvent._id,data:{organizationConfirmed:$scope.globalEvent.organizationsPending[index]}}).then(function(res){console.log("Successful accept"),$scope.loadEventList(),console.log(index),console.log($scope.globalEvent),console.log($scope.globalEvent.organizationsPending[index])},function(res){console.log("Failed accept"),console.log(res)})},$scope.displayOrgNonRequest=function(event){return console.log($scope.eventList),console.log(event.organizationsPending===[]),event.organizationsPending===[]||event.organizationsPending.indexOf($scope.authentication.user.displayName)===-1},$scope.deleteOrgRequest=function(event){console.log(event.organizationsPending.splice(event.organizationsPending.indexOf($scope.authentication.user.displayName),1));var newConfirmed=event.organizationConfirmed;newConfirmed===$scope.authentication.user.displayName&&(newConfirmed=""),$http({method:"PUT",url:"api/events/"+event._id,data:{organizationsPending:event.organizationsPending.splice(event.organizationsPending.indexOf($scope.authentication.user.displayName),1),organizationConfirmed:newConfirmed}}).then(function(res){console.log("Successful org event delete")},function(res){console.log("Failed org event delete"),console.log(res)})},$scope.loadEventList=function(){$http({method:"GET",url:"/api/events"}).then(function(res){console.log("Successful"),console.log(res),$scope.eventList=res.data},function(res){console.log("Failed"),console.log(res)}),console.log($scope.eventList[0].name);for(var i=0;i<$scope.eventList.length-1;i++)$scope.addEvent($scope.eventList[i].name)},$scope.loadEventList(),$scope.filterByUser=function(event){return event.user.displayName===$scope.authentication.user.displayName},$scope.filterOrgEvents=function(event){return event.organizationsPending.indexOf($scope.authentication.user.displayName)!==-1},$scope.createEvent=function(){console.log($scope.name),console.log($scope.date),console.log($scope.sTime),$http({method:"POST",url:"/api/events",data:{name:$scope.name,dateOfEvent:$scope.date,startTime:$scope.sTime,endTime:$scope.eTime,location:$scope.location,taxIdRequired:$scope.requireTax}}).then(function(res){console.log("Successful event")},function(res){console.log("Failed event"),console.log(res),console.log(name)}),$scope.name=null,$scope.date=null,$scope.sTime=null,$scope.eTime=null,$scope.location=null,$scope.requireTax=null},$scope.toggleAcceptFlag=function(){$scope.acceptEvent_flag=!$scope.acceptEvent_flag,console.log($scope.acceptEvent_flag)},$scope.setGlobalEvent=function(event){console.log("setting event"),$scope.globalEvent=event},$scope.isOrg=!1,$scope.isBiz=!1,""===$scope.authentication.user?$scope.isOrg=!1:$scope.authentication.user.roles.indexOf("Organization")>=0?$scope.isOrg=!0:$scope.isOrg=!1,""===$scope.authentication.user?$scope.isBiz=!1:$scope.authentication.user.roles.indexOf("Business")>=0?$scope.isBiz=!0:$scope.isBiz=!1,console.log($scope.isBiz),console.log($scope.isOrg);var date=new Date;date.getDate(),date.getMonth(),date.getFullYear();$scope.eventRender=function(event,element,view){element.attr({tooltip:event.title,"tooltip-append-to-body":!0}),$compile(element)($scope)},$scope.uiConfig={calendar:{height:450,editable:!0,header:{left:"title",center:"",right:"today prev,next"},eventRender:$scope.eventRender}},$scope.eventSources=[$scope.events]}]);
...
"use strict";angular.module("core").controller("HomeController",["$scope","$http","Authentication",function($scope,$http,Authentication){$scope.authentication=Authentication}]);
...
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("events",{"abstract":!0,url:"/events",template:"<ui-view/>"}).state("events.list",{url:"",templateUrl:"modules/events/client/views/list-events.client.view.html",controller:"EventsListController",controllerAs:"vm",data:{pageTitle:"Events List"}}).state("events.create",{url:"/create",templateUrl:"modules/events/client/views/form-event.client.view.html",controller:"EventsController",controllerAs:"vm",resolve:{eventResolve:newEvent},data:{roles:["Business"],pageTitle:"Events Create"}}).state("events.edit",{url:"/:eventId/edit",templateUrl:"modules/events/client/views/form-event.client.view.html",controller:"EventsController",controllerAs:"vm",resolve:{eventResolve:getEvent},data:{roles:["Business"],pageTitle:"Edit Event {{ eventResolve.name }}"}}).state("events.view",{url:"/:eventId",templateUrl:"modules/events/client/views/view-event.client.view.html",controller:"EventsController",controllerAs:"vm",resolve:{eventResolve:getEvent},data:{pageTitle:"Event {{ eventResolve.name }}"}})}function getEvent($stateParams,EventsService){return EventsService.get({eventId:$stateParams.eventId}).$promise}function newEvent(EventsService){return new EventsService}angular.module("events").config(routeConfig),routeConfig.$inject=["$stateProvider"],getEvent.$inject=["$stateParams","EventsService"],newEvent.$inject=["EventsService"]}();
!function(){"use strict";function EventsController($scope,$state,$window,Authentication,event){function remove(){$window.confirm("Are you sure you want to delete?")&&vm.event.$remove($state.go("events.list"))}function save(isValid){function successCallback(res){$state.go("events.view",{eventId:res._id})}function errorCallback(res){vm.error=res.data.message}return isValid?void(vm.event._id?vm.event.$update(successCallback,errorCallback):vm.event.$save(successCallback,errorCallback)):($scope.$broadcast("show-errors-check-validity","vm.form.eventForm"),!1)}var vm=this;vm.authentication=Authentication,vm.event=event,vm.error=null,vm.form={},vm.remove=remove,vm.save=save}angular.module("events").controller("EventsController",EventsController),EventsController.$inject=["$scope","$state","$window","Authentication","eventResolve"]}();
!function(){"use strict";function EventsListController(EventsService){var vm=this;vm.events=EventsService.query()}angular.module("events").controller("EventsListController",EventsListController),EventsListController.$inject=["EventsService"]}();
!function(){"use strict";function EventsService($resource){return $resource("api/events/:eventId",{eventId:"@_id"},{update:{method:"PUT"}})}angular.module("events").factory("EventsService",EventsService),EventsService.$inject=["$resource"]}();
